...
impl DependencyGraph {
    ...
    pub fn add_symbol_reference(&mut self, from: &Path, to: &Path, edge_type: EdgeType) {
    ...
    pub fn calculate_importance(&self, focused_paths: &[PathBuf]) -> BTreeMap<PathBuf, f64> {
        // Changed return type to BTreeMap
        ...
        let nodes: Vec<_> = self.graph.node_indices().collect::<Vec<_>>(); // Collect once and reuse
        ...
        for &node_idx in &nodes {
            if let Some(path) = self.graph.node_weight(node_idx) {
                ...
                let multiplier = if focused_paths.contains(path) {
                    focus_boost
                } else {
                    1.0
                };
    ...
    pub fn debug_print_state(&self) {
        ...
        for (path, weight) in &self.total_weights {
