...
impl RepoMap {
    ...
    pub async fn process_files(&mut self, source_files: Vec<(PathBuf, String)>) -> Result<()> {
        ...
        for (path, content) in &source_files {
            ...
            for symbol in &symbols {
                ...
                if symbol.is_public() {  // Only track public symbols
    ...
    fn build_dependency_graph(&mut self) {
        ...
        for (file_path, symbols) in &self.files {
            ...
            for symbol in symbols {
                println!("  Processing symbol {} with {} references", symbol.name, symbol.references.len()); // Debug
                ...
                for reference in &symbol.references {
                    ...
                    if !added_edges.contains(&edge) {
                        ...
                        let edge_type = match symbol.kind {
                            SymbolKind::Function | SymbolKind::Method => crate::ranking::EdgeType::Call,
                            SymbolKind::Struct | SymbolKind::Class => crate::ranking::EdgeType::Implementation,
                            SymbolKind::Module => crate::ranking::EdgeType::Import,
                            _ => crate::ranking::EdgeType::Use,
                        };

                        println!("    Adding reference: {} -> {} ({:?})", 
                            from_path.display(), 
                            to_path.display(), 
                            edge_type);

                        self.graph.add_symbol_reference(
                            from_path,
                            to_path,
                            edge_type,
                        );
    ...
    pub fn get_context(&self, focused_paths: &[PathBuf]) -> Result<String> {
        ...
        let mut files: Vec<(&PathBuf, &Vec<Symbol>)> = self.files.iter()
...
mod tests {
    ...
    async fn test_process_single_file() -> Result<()> {
        ...
        repo_map.process_files(vec![(PathBuf::from("test.rs"), content)]).await?;
    ...
    async fn test_directory_context_snapshot() -> Result<()> {
        ...
        repo_map = repo_map.with_config(RepoMapConfig {
            token_budget: 4000,
            ..Default::default()
        });
        ...
        let file_map: Vec<(PathBuf, String)> = file_map.into_iter().map(|(_, v)| v).collect();
