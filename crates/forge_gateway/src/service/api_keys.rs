...
impl ApiKeyService {
    ...
    pub async fn create_api_key(&self, user_id: &str, key_name: &str) -> Result<ApiKey> {
        ...
        let api_key = ApiKey::new(user_id.to_string(), key_name.to_string(), key_gen);
        self.repo.save(api_key.clone()).await
    }
    ...
    pub async fn get_by_key_id(&self, user_id: &str, key_id: Uuid) -> Result<ApiKey> {
        match self.repo.get_by_key_id(user_id, key_id).await? {
            Some(maybe_key) => Ok(maybe_key),
            None => Err(Error::NotFound("API key not found".to_string())),
        }
    }
