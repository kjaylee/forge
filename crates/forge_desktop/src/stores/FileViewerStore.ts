...
export const useFileViewerStore = create<FileViewerState>((set, get) => ({
  ...
  openFile: async (filePath) => {
    try {
      ...
      const fileInfo = await invoke<{ extension: string; size: number }>('get_file_info', { path: cleanPath });
      
      // Check file size against limit
      if (get().maxFileSize > 0 && fileInfo.size > get().maxFileSize) {
        throw new Error(`File is too large (${Math.round(fileInfo.size / 1024 / 1024)}MB). Maximum size is ${Math.round(get().maxFileSize / 1024 / 1024)}MB.`);
      }
      
      const language = determineLanguage(fileInfo.extension);
      
      // Read file content using our Rust command
      const content = await invoke<string>('read_file_content', { path: cleanPath });
