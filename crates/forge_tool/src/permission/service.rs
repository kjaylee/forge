...
impl LivePermissionService {
    ...
    pub async fn check_permission(&self, perm: Permission, cmd: Option<&str>) -> PermissionResult<bool> {
...
mod tests {
    ...
    async fn test_once_permissions() {
        ...
        let execute = service.check_permission(Permission::Execute, Some("ls")).await;

        assert!(matches!(read, Ok(false)), "Read should require asking");
        assert!(matches!(write, Ok(false)), "Write should require asking");
        assert!(matches!(execute, Ok(false)), "Execute should require asking");
    }
    ...
    async fn test_execute_whitelist() {
        ...
        // Whitelisted command
        let result = service.check_permission(Permission::Execute, Some("ls -la")).await;
        assert!(matches!(result, Ok(true)), "Whitelisted command should be allowed");

        // Non-whitelisted command  
        let result = service.check_permission(Permission::Execute, Some("rm -rf")).await;
        assert!(matches!(result, Ok(false)), "Non-whitelisted command should be denied");
        
        // Missing command
        let result = service.check_permission(Permission::Execute, None).await;
        assert!(matches!(result, Ok(false)), "Missing command should be denied");
    }
    ...
    async fn test_always_read() {
        ...
        assert!(matches!(result, Ok(true)), "Always policy should allow read");
    }
}