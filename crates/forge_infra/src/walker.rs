...
impl Default for ForgeWalkerService {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ForgeWalkerService {
    ...
    pub async fn walk(&self, config: Walker) -> Result<Vec<WalkedFile>> {
        // Convert domain config to internal walker config
        let mut walker = if config.max_depth.is_none()
            ...
            && !config.skip_binary
        {
            InternalWalker::max_all()
        } else {
            InternalWalker::min_all()
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn test_internal_walker_respects_file_size_limit() {
        ...
        let actual = InternalWalker::min_all()
    ...
    #[tokio::test]
    async fn test_internal_walker_filters_binary_files() {
        let fixture =
    ...
    #[tokio::test]
    async fn test_internal_walker_enforces_directory_breadth_limit() {
        let (fixture, _) =
    ...
    #[tokio::test]
    async fn test_internal_walker_enforces_directory_depth_limit() {
