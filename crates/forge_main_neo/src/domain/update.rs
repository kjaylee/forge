...
pub fn update(state: &mut State, action: impl Into<Action>) -> Command {
    ...
    match action {
        ...
        Action::ShowInfo => {
            ...
            // Hide spotlight if it's visible
            if state.show_info {
...
mod tests {
    ...
    #[test]
    fn test_show_info_action_toggles_info_popup() {
        let mut fixture = State::default(test_env());
        let action = Action::ShowInfo;

        // Initially info should be hidden
        assert_eq!(fixture.show_info, false);

        let actual = update(&mut fixture, action.clone());

        // After first call, info should be visible
        assert_eq!(fixture.show_info, true);
        assert_eq!(actual, Command::Empty);

        // Call again to toggle off
        let actual = update(&mut fixture, action);

        // After second call, info should be hidden again
        assert_eq!(fixture.show_info, false);
        assert_eq!(actual, Command::Empty);
    }

    #[test]
    fn test_show_info_action_hides_spotlight() {
        let mut fixture = State::default(test_env());
        ...
        // Info should be visible and spotlight should be hidden
        assert_eq!(fixture.show_info, true);
        assert_eq!(fixture.spotlight.is_visible, false);
        assert_eq!(actual, Command::Empty);
    }

    #[test]
    fn test_update_processes_key_press_events() {
        let mut fixture_state = State::default(test_env());
