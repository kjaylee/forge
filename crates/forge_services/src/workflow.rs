...
impl<F: Infrastructure> ForgeWorkflowService<F> {
    ...
    /// Returns the path to the first found config file, or the original path if none is found.
    async fn find_config_file(&self, path: &Path) -> PathBuf {
    ...
    /// If the file doesn't exist anywhere, creates a new empty workflow file at the
    /// specified path (in the current directory).
    pub async fn read(&self, path: &Path) -> anyhow::Result<Workflow> {
        // First, try to find the config file in parent directories if needed
        let config_path = self.find_config_file(path).await;
        
        if !config_path.exists() {
            let workflow = Workflow::new();
            self.infra
                .file_write_service()
                .write(&config_path, serde_yml::to_string(&workflow)?.into())
                .await?;

            Ok(workflow)
        } else {
            let content = self.infra.file_read_service().read(&config_path).await?;
            let workflow: Workflow = serde_yml::from_str(&content)
                .with_context(|| format!("Failed to parse workflow from {}", config_path.display()))?;
            Ok(workflow)
        }
    }
}
...
impl<F: Infrastructure> WorkflowService for ForgeWorkflowService<F> {
    ...
    async fn write(&self, path: &Path, workflow: &Workflow) -> anyhow::Result<()> {
        ...
        self.infra
            .file_write_service()
            .write(&config_path, content.into())
            .await
    }
...
mod tests {
    ...
    fn test_find_config_file_behavior() {
        ...
        let result = find_config_file_logic(
            &custom_path, 
            &temp_dir.path().join("file-that-exists.txt")
        );
    ...
    fn find_config_file_logic(path: &Path, existing_config_path: &Path) -> PathBuf {
        ...
        if existing_config_path.file_name().unwrap_or_default() == path.file_name().unwrap_or_default() {
