...
/// Use for project plans, RFCs, or migration strategies. Filenames follow "YYYY-MM-DD-name-v{version}.md",
/// with versions incrementing for same-named plans on the same day. Only writes markdown files to the plans
/// directory and cannot modify existing files. Content should be complete markdown. Plans directory is
/// created if needed. Names are sanitized (spaces to hyphens, special characters removed).
