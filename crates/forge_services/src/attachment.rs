    ...
    async fn prepare_attachments<T: AsRef<Path>>(
        ...
        let results = futures::future::join_all(
            paths
                ...
                .map(|v| async move {
                    match self.populate_attachments(v).await {
...
pub mod tests {
    ...
    async fn test_add_url_with_nonexistent_file() {
        ...
        // Execute - With our new implementation, this should succeed with an empty result
        ...
        assert!(attachments.is_empty(), "Expected empty attachments list for non-existent file");
    }
